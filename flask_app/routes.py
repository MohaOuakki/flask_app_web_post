...
def register():
    ...
    if form.validate_on_submit():
        ...
        db.session.add(user)
        db.session.commit()
        flash('Your Account have successfully created! You are now able to login', 'success')
...
def login():
    ...
    if form.validate_on_submit():
        ...
        if user and bcrypt.check_password_hash(user.password, form.password.data):
            ...
            flash('Login not have been good , Please check email and password', 'danger')    
...
def new_post():
    ...
    if form.validate_on_submit():
        ...
        db.session.add(post)
        db.session.commit()
        flash('You post have been Uploaded!', 'success')
        return redirect(url_for('home'))
...
def send_reset_email(user):
    ...
    msg = Message('Password Reset Request',
                  sender='micheladii464@gmail.com',
